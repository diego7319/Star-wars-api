...
def obtener_peliculas():
    ...
    siguiente_pagina = "https://swapi.info/api/films/"
    json_data = json.loads((requests.get(siguiente_pagina)).content)
    for json_pelicula in json_data:
...
def obtener_personajes():
    lista_personajes = []
    #Api de multiples paginas
    siguiente_pagina = "https://swapi.info/api/people/"
    json_data = json.loads((requests.get(siguiente_pagina)).content)
    for personaje_json in json_data:
...
def obtener_personajes_por_planeta(num_planeta):
    dict_personajes = {}
    lista_personajes = []
    api_base = "https://swapi.info/api/planets/" + str(num_planeta)
    #Api de multiples paginas
...
def obtener_peliculas_por_personajes(num_planeta):
    dict_personajes = {}
    lista_personajes = []
    api_base = "https://swapi.info/api/planets/" + str(num_planeta)
    #Api de multiples paginas
...
def _obtener_numero_id(dato):
    dato_len=len(dato)
    return dato.rstrip('/').split('/')[-1]#dato[31:dato_len-1]
